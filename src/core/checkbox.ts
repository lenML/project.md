 ...
 */
export async function checkbox_toggle(item_path: string, ...hashes: string[]): Promise<void> {
  if (hashes.length === 0) return;

  const kanban_dir = get_kanban_dir_from_item(item_path) ?? undefined;
  const hook_ctx = { item_path };
  const hook_result = await run_before_hook(kanban_dir, 'before_checkbox_toggle', hook_ctx);
  if (hook_result) {
