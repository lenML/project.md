...
export async function item_new(
  ...
  const safe_name = Array.from(name)
    .map((ch) => (ch < ' ' ? '_' : ch))
    .join('')
    .replace(/[\s<>:"/\\|?*]/g, '_');
...
async function parse_item_frontmatter(
  file_path: string,
  entry_name: string,
...
export async function item_import(column_dir: string, source_file: string): Promise<ItemSummary> {
  ...
  if (parsed) {
    metadata = {
      ...parsed.metadata,
      id,
      name: (parsed.metadata.name as string) || source_basename,
      created_at: now,
    };
    body = parsed.body;
  } else {
