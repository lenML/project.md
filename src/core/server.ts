...
export async function start_server(opts: ServerOptions): Promise<void> {
  ...
  const srv = createServer(async (req, res) => {
    try {
      ...
      try {
        if (body) body_json = JSON.parse(body);
      } catch {
        /* ignore */
      }
      ...
      if (parts[0] === 'items' && parts.length === 1 && method === 'GET') {
        ...
        if (!proj || !kanban || !col) {
          error(res, 'need project+kanban+col');
          return;
        }
      ...
      if (parts[0] === 'item' && parts.length === 1 && method === 'POST') {
        ...
        if (!proj || !kanban || !col || !name) {
          error(res, 'need project+kanban+col+name');
          return;
        }
      ...
      if (parts[0] === 'checkbox' && parts[1] === 'toggle' && method === 'POST') {
        ...
        if (!item_path || hashes.length === 0) {
          error(res, 'need item_path+hashes');
          return;
        }
