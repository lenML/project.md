...
import { validate_checkboxes_all_done, item_move_with_check } from '../../src/core/checklist.js';
...
describe('item_move_with_check', () => {
  ...
  it('blocks move to done when checkboxes incomplete', async () => {
    ...
    await expect(item_move_with_check(item.file_path, column_dst)).rejects.toThrow(
      '未完成的 checkbox',
    );
  });
  ...
  it('executes custom hook from .hooks/ directory', async () => {
    ...
    await expect(item_move_with_check(item.file_path, column_dst)).rejects.toThrow('hook 拦截');
  });
