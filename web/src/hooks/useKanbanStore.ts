...
export function useKanban() {
  ...
  const project = useMemo(() => projects.find((p) => p.name === view.project), [projects, view.project]);
  const kanban = useMemo(() => project?.kanbans.find((k) => k.name === view.kanban), [project, view.kanban]);
  ...
  const onDrop = useCallback((colName: string) => {
  ...
  return { projects, view, setView, moveCard, writeMode, searchQuery, setSearchQuery, loadTrash, CARD_PAGE_SIZE, project, kanban, sortedColumns, displayedColumns, onDrop };
}