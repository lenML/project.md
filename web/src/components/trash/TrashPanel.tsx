...
function TrashItemRow({
  ...
  return (
    <div className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-2">
      ...
      <button
        onClick={() => restoreFromTrash(project, kanban, targetCol, item.path)}
      ...
      <button
        onClick={() => {
          if (confirm('永久删除?')) purgeFromTrash(item.path);
        }}
...
export default function TrashPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg max-h-[70vh] bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden"
