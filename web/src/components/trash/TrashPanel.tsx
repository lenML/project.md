...
interface TrashItem {
  name: string; path: string; originalName: string;
}

function TrashItemRow({ item, project, kanban }: {
  item: TrashItem; project: string; kanban: string;
}) {
  ...
  return (
    <div className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-2">
      ...
      <select className="text-xs bg-slate-700 border border-slate-600 rounded px-1.5 py-1 text-slate-300"
      ...
      <button onClick={() => restoreFromTrash(project, kanban, targetCol, item.path)}
        className="text-indigo-400 hover:text-indigo-300 transition-colors p-1" title="恢复">
        <RotateCcw size={14} />
      </button>
      <button onClick={() => { if (confirm("永久删除?")) purgeFromTrash(item.path); }}
        className="text-red-400 hover:text-red-300 transition-colors p-1" title="永久删除">
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-12 bg-black/60" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[70vh] bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-2 overflow-y-auto max-h-[55vh]">
          {trashItems.length === 0 && <div className="text-sm text-slate-500 text-center py-8">回收站为空</div>}
          {trashItems.map((item) => (
            <TrashItemRow key={item.path} item={item} project={view.project!} kanban={view.kanban!} />
          ))}
        </div>
      </div>
    </div>
